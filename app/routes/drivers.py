...
def get_all_drivers():
    ...
    for driver in drivers:
        response.append(driver.to_dict())
...
def get_one_driver_or_abort(driver_id):
    ...
    chosen_driver = Driver.query.get(driver_id) #"get" only works with primary key, otherwise have to filter a different way
...
def add_cars_to_driver(driver_id):
    ...
    if not isinstance(car_ids, list):
        return jsonify({'msg': "Expected list of cars ids"}), 400
    ...
    for car in cars:
        ...
        car.driver = driver #an alternative to the above line
